...
# 策略类型: 套利策略
# 核心指标: 布林带(BOLL), 价比
# 交易逻辑: 价比突破布林带下轨→多豆油空棕榈油(做多价差); 价比突破布林带上轨→空豆油多棕榈油(做空价差)
...
def handlebar(ContextInfo):
    ...
    if ContextInfo.position_state == 0:
        if current_spread < lower:
            ...
            print("做多价差: 多豆油空棕榈油 %d手, 价比=%.4f" % (target_volume, current_spread))
            ...
            print("做空价差: 空豆油多棕榈油 %d手, 价比=%.4f" % (target_volume, current_spread))
