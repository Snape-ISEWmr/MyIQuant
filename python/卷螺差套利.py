...
# 策略类型: 套利策略
# 核心指标: 布林带(BOLL), 价比
# 交易逻辑: 价比突破布林带下轨→多螺纹空热卷(做多价差); 价比突破布林带上轨→空螺纹多热卷(做空价差)
# 风控方式: 浮亏15%减仓 + 浮亏25%全平 + 保证金上限10%
...
def init(ContextInfo):
    ContextInfo.symbol_1 = 'rb2609.SHF'
    ContextInfo.symbol_2 = 'hc2609.SHF'
...
def handlebar(ContextInfo):
    ...
    margin_ratio_1_long = float(inst1.get('LongMarginRatio', 0.10))
    margin_ratio_1_short = float(inst1.get('ShortMarginRatio', 0.10))
    margin_ratio_2_long = float(inst2.get('LongMarginRatio', 0.10))
    margin_ratio_2_short = float(inst2.get('ShortMarginRatio', 0.10))
    ...
    if ContextInfo.position_state == 0:
        if current_spread < lower:
            ...
            print("做多价差: 多螺纹空热卷 %d手, 价比=%.4f" % (target_volume, current_spread))
            ...
            print("做空价差: 空螺纹多热卷 %d手, 价比=%.4f" % (target_volume, current_spread))
